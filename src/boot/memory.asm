...
%macro m_bios_get_memory_size_E801 0
	...
	cmp		ah, 0x80		; Invalid command
	je		short .error
	jcxz	short .use_ax	; BIOS may have stored it in AX, BX or CX, DX. Test if CX is 0
...
; Output:
;	SI - The number of memory map entries.
;
; Need to check what to push then pop.
%macro m_bios_get_memory_map 0
	pushad
	
	xor		ebx, ebx			; Start as 0, must preserve value after INT 0x15
	xor		si, si				; Number of entries
	...
	jc		short .error		; If carry is set, then there was and error
	cmp		eax, 0x534D4150		; BIOS returns SMAP in EAX
	jne		short .error
	test	ebx, ebx			; If EBX = 0 then list is one entry
	je		short .error		; Then is worthless, so error.
	jmp		short .start
.next_entry:
	mov		eax, 0x0000E820
	mov		ecx, 24
...
.start:
	jcxz	short .skip_entry	; If actual returned bytes is 0, skip entry
	cmp		cl, 20				; Has it returned a a 24 byte ACPI 3.x response
	jbe		short .notext
	test	byte [es:di + 20], 1	; If so, is the 'ignore this data' bit set
	jc		short .skip_entry
.notext:
	...
	jz		.skip_entry				; If zero, the skip entry
	inc		si						; Good entry so increment entry count and buffer offset
	add		di, 24
.skip_entry:
